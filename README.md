...
- 位置采集使用更高精度配置；前台位置上报和后台位置服务频率已提高，并按时间和精度选择更可靠坐标。
- Android 定位坐标会转换为高德地图可用坐标，避免真实位置和地图展示出现偏移。
- 地图支持手指缩放、实时定位卡片全屏查看，并默认使用更近的缩放级别。
- Android 前台服务会定期随位置上报设备状态快照，包括电量、网络、蓝牙、音量、存储和可用流量信息。
- 总览下拉刷新会先同步本机遥测，再刷新对方概览。
- 底部导航为 `总览 / 应用 / 记录 / 我的`；绑定入口在 `我的` 页，隐私和账号设置在 `我的 -> 设置`。
- `我的` 页头像可进入编辑资料，支持头像、用户名、今日心情状态和性别。
- `应用` 页保留底部导航入口，空数据时会说明未绑定、未授权、未同步或今日暂无会话。
...
- 优化实时定位：按时间和精度选择更可靠位置，并将 Android WGS-84 坐标转换为高德地图可用坐标。
- 实时定位卡片新增全屏地图查看，保留位置详情和精度范围。
- Android 前台服务定期补充设备状态快照，包含电量、网络、蓝牙、音量、存储和可用流量信息。
- 总览刷新会先同步本机状态，再拉取对方概览。
- 验证：`currentVersionCode=2011` 返回 `0.2.9 / 2012`，`currentVersionCode=2012` 返回 `updateAvailable: false`。

### 0.2.8 (2011)

- 修复应用页空数据体验，显示未绑定、对方未授权、对方未同步或今日暂无会话等原因，并保留刷新和本机授权入口。
- 新增编辑资料页和公开头像上传，支持头像、用户名、今日心情状态、性别；总览问候语改为当前用户姓名，关系卡标题改为双方姓名。
- 设置页把隐私状态和权限与可用性改为默认收起的折叠区，移除数据范围区块。
- 验证：`currentVersionCode=2010` 返回 `0.2.8 / 2011`，`currentVersionCode=2011` 返回 `updateAvailable: false`。
